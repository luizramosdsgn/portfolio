document.addEventListener("DOMContentLoaded", () => {
	...
	const brilho2Buttons = document.querySelectorAll(
	...
	brilho2Buttons.forEach((brilho2Button, index) => {
		...
		if (brilho1 && brilho2Button) {
			brilho2Button.addEventListener("mousemove", (event) => {
				...
				const mousePercentage = Math.min(
					Math.max(mouseX / brilho1Rect.width, 0),
					1,
				);

				brilho1.style.setProperty(
					"--before-opacity",
					mousePercentage.toFixed(2),
				);
				brilho1.style.setProperty(
					"--after-opacity",
				...
				brilho2Button.style.setProperty(
					"--button-translate-x",
					`${translateX}%`,
				);
			});
